...
from get_size_object import get_size
import unittest
import lab
import json
    ...
    class TestQueryMedium(unittest.TestCase):
        ...
        def test_03(self):
            ...
            with open('resources/expected_cliques.json', 'r') as f:
                expected = json.load(f)
            with open("resources/labels_cliques.json", 'r') as f:
                labels = json.load(f)
            for i, n in enumerate((3, 4, 5, 6)):
                graph = self.graph1
                if i % 2 == 1:
        ...
        def test_04(self):
            ...
            with open('resources/expected_paths.json', 'r') as f:
                expected = json.load(f)
            with open("resources/labels_paths.json", 'r') as f:
                labels = json.load(f)
...
class Test_7_MemoryTest(unittest.TestCase):
    def setUp(self):
        self.adj_list = [[i for i in range(1000)] for j in range(1000)]

    def test_largeGraph(self):
        # Only tests for memory limit.
        graph = lab.GraphFactory(lab.CompactGraph).from_list(self.adj_list)
        self.assertLessEqual(get_size(graph), 400000, "Instance of Compact Graph is too big, so not space efficient enough")
